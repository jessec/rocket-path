 ...
 * annotated value object will belong to. Since the node value object is known (instance of the annotated class), this
 * annotation describes the key (reference to a key object) and the possible child nodes (references to their value
 * objects).
 * <p>
 * For both node key and child node values it is possible to use either reference by bean name or reference by type.
 * When key attributes are omitted, the tree node key will be the CDI bean name (String) of the node value. When
 * children attributes are omitted, the constructed tree node will not have any children.
 * 
...
public @interface TreeNode {
  ...
  /**
   * Reference to node key object by bean type. This value is used when <code>keyType != Object.class</code>. When CDI
